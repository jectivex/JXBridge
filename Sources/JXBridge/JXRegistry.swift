...
public class JXRegistry {
    ...
    @discardableResult public func add<T: JXStaticBridging>(for type: T.Type) throws -> JXBridge {
        ...
        if let bridgingSuperclass = bridge.superclass as? any JXStaticBridging.Type, self.bridge(for: bridgingSuperclass) == nil {
            try add(for: bridgingSuperclass)
        }
        try add(bridge)
        return bridge
    }
    ...
    @discardableResult public func add<T: JXBridging>(for instance: T) throws -> JXBridge {
        let mirror = Mirror(reflecting: instance)
        return try add(for: T.self, mirror: mirror)
    }
    
    @discardableResult private func add<T: JXBridging>(for type: T.Type, mirror: Mirror) throws -> JXBridge {
        ...
        if let superclass = bridge.superclass, self.bridge(for: superclass) == nil {
            if let bridgingSuperclass = superclass as? any JXStaticBridging.Type {
                ...
                try add(for: bridgingSuperclass, mirror: superclassMirror)
            }
        }
        try add(bridge)
        return bridge
