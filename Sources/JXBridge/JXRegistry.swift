...
public class JXRegistry {
    ...
    @discardableResult public func registerBridge<T: JXStaticBridging>(for type: T.Type) throws -> JXBridge {
        let bridge = try T.jxBridge()
    ...
    @discardableResult public func registerBridge<T: JXBridging>(for instance: T) throws -> JXBridge {
        let mirror = Mirror(reflecting: instance)
        return try registerBridge(for: T.self, mirror: mirror)
    }
    
    @discardableResult private func registerBridge<T: JXBridging>(for type: T.Type, mirror: Mirror) throws -> JXBridge {
        ...
        if let superclass = bridge.superclass, self.bridge(for: superclass) == nil {
            if let bridgingSuperclass = superclass as? any JXStaticBridging.Type {
                ...
                try registerBridge(for: bridgingSuperclass, mirror: superclassMirror)
            }
        }
        try register(bridge)
        return bridge
