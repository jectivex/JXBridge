...
@main struct BridgeGeneratorTool: BuildToolPlugin {
    func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
        ...
        return [
            .buildCommand(displayName: "JXBridgeBuilder", executable: bridgeGenerator, arguments: ["-o", outputFile, "-d", inputDir], outputFiles: [outputFile])
        ]
    }
}
...
extension BridgeGeneratorTool: XcodeBuildToolPlugin {
    func createBuildCommands(context: XcodePluginContext, target: XcodeTarget) throws -> [Command] {
        ...
        let arguments = target.inputFiles
            ...
            .map { $0.path.string }
        return [
            .buildCommand(displayName: "JXBridgeBuilder", executable: bridgeGenerator, arguments: ["-o", outputFile] + arguments, outputFiles: [outputFile])
        ]
    }
}
#endif
